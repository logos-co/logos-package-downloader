#!/usr/bin/env bash
#
# Execute every logos-package-downloader doc-test end-to-end and regenerate its Markdown.
#
...
# published flake. Each spec's `github:logos-co/logos-package-downloader{release}` URL is pinned to
...
# to logos-co/logos-package-downloader. A local-only / uncommitted HEAD won't resolve; export
# COMMIT="" (or push first) in that case.
COMMIT="${COMMIT-$(git rev-parse HEAD)}"
RELEASE_FOR=()
if [ -n "${COMMIT}" ]; then
  RELEASE_FOR=(--release-for "logos-package-downloader=${COMMIT}")
  echo "==> Pinning logos-package-downloader to ${COMMIT}"
else
  echo "==> COMMIT empty; building logos-package-downloader from latest"
fi
