...
| `download` | `lgpd [--repo <url-or-name>] [--version <ver>] [--root-hash <hex>] [-o\|--output <dir>] download <package>` | Download a `.lgx`. `repo` empty = any enabled; `version` empty = newest; `root-hash` disambiguates same-version builds; `output` empty = a private per-user staging dir under the system temp dir (`TMPDIR` or `/tmp`), e.g. `/tmp/lgpd-1000/`. Verifies against the catalog before printing the local path. Exit 1 on failure (prints `FAILED`). |
